...
export default function MealPrepScreen({ navigation }) {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const searchQuery = "healthy meal prep recipes short video";
        const response = await axios.get(
          `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoDuration=short&maxResults=20&q=${encodeURIComponent(searchQuery)}&key=${YOUTUBE_API_KEY}`
        );

        const videoData = response.data.items.map(item => ({
          id: item.id.videoId,
          title: item.snippet.title,
          videoUrl: `https://www.youtube.com/embed/${item.id.videoId}?autoplay=1&mute=0&loop=1&controls=0&playsinline=1`,
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topNav}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('MealPrepSearch')}>
